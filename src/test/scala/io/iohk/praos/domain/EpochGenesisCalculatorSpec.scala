...
class EpochGenesisCalculatorSpec extends FeatureSpec with GivenWhenThen {
  ...
  feature("compute genesis for each slot in epoch") {
    scenario("compute the correct genesis for the first 5 slots") {
      ...
      assert (epochGenesisCalculator.computeGenesisForEpoch(slot1.epochNumber, genesisHistory).contains(initialGenesis))

      val slot2 = SlotInEpoch(epochNumber = 1, slotNumber = 2, firstInEpoch = false)
      assert (epochGenesisCalculator.computeGenesisForEpoch(slot2.epochNumber, genesisHistory).contains(initialGenesis))

      val slot3 = SlotInEpoch(epochNumber = 1, slotNumber = 3, firstInEpoch = false)
      assert (epochGenesisCalculator.computeGenesisForEpoch(slot3.epochNumber, genesisHistory).contains(initialGenesis))

      val slot4 = SlotInEpoch(epochNumber = 1, slotNumber = 4, firstInEpoch = false)
      assert (epochGenesisCalculator.computeGenesisForEpoch(slot4.epochNumber, genesisHistory).contains(initialGenesis))
      ...
      assert (epochGenesisCalculator.computeGenesisForEpoch(slot5.epochNumber, genesisHistory).contains(newGenesis))
    }
    ...
    scenario("throw an exception if the genesis for the given slot isn't initialized yet") {
      ...
      intercept[NoSuchElementException] {
        epochGenesisCalculator.computeGenesisForEpoch(slot1.epochNumber, genesisHistory).get
      }

      val slot2 = SlotInEpoch(epochNumber = 1, slotNumber = 2, firstInEpoch = false)
      intercept[NoSuchElementException] {
        epochGenesisCalculator.computeGenesisForEpoch(slot2.epochNumber, genesisHistory).get
      }

      val slot3 = SlotInEpoch(epochNumber = 1, slotNumber = 3, firstInEpoch = false)
      intercept[NoSuchElementException] {
        epochGenesisCalculator.computeGenesisForEpoch(slot3.epochNumber, genesisHistory).get
      }

      val slot4 = SlotInEpoch(epochNumber = 1, slotNumber = 4, firstInEpoch = false)
      intercept[NoSuchElementException] {
        epochGenesisCalculator.computeGenesisForEpoch(slot4.epochNumber, genesisHistory).get
      }

      val slot5 = SlotInEpoch(epochNumber = 2, slotNumber = 5, firstInEpoch = true)
      intercept[NoSuchElementException] {
        epochGenesisCalculator.computeGenesisForEpoch(slot5.epochNumber, genesisHistory).get
      }
    }
  }
}