...
class cryptoSpec extends FlatSpec with Matchers {
  ...
  it should "get the user public key from its private key" in new cryptoTestSetup {
    getPublicKeyFromPrivateKey(userPrivateKey) should be (userPublicKey)
  }

  "Cipher" should "encrypt a message using user's pk and then decrypt them using user's sk correctly" in new cryptoTestSetup {
  ...
  "Cipher" should "NOT decrypt an encrypted message using user's pk" in new cryptoTestSetup {
    val message = serialize("a secret message")
    cipher.decryptWith(cipher.encryptWith(message, userPublicKey), userPublicKey) shouldNot be (Some(message))
  }

  "Signer" should "sign a message using user's sk and then strip the signed message correctly" in new cryptoTestSetup {
  ...
  "Signer" should "NOT sign a message using user's pk and then strip the signed message correctly" in new cryptoTestSetup {
    val message = serialize("I'm stakeholder Ua")
